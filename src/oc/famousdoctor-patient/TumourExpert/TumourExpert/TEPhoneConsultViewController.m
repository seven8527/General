//
//  TEPhoneConsultViewController.m
//  TumourExpert
//
...
@interface TEPhoneConsultViewController () 
...
@property BOOL isPhoneSelected; // 是否勾选电话咨询协议
@property BOOL isAgreementSelected; // 是否勾选同意提示信息
...
@implementation TEPhoneConsultViewController
...
- (UITableViewCell *)tableView:(UITableView *)aTableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        ...
        if (indexPath.row == 0) {
            ...
            //            cell.detailTextLabel.text = self.patientDataName;
...
- (void)fetchAsk
{
    ...
    NSDictionary *parameters = @{@"doctorid": self.expertId, @"type": @"1"};
    [TEHttpTools post:URLString params:parameters success:^(id responseObject) {
        NSLog(@"%@", responseObject);
...
- (BOOL)validatePhone:(NSString *)phone patientId:(NSString *)patientId patientDataId:(NSString *)patientDataId selectHealthFiles:(NSArray *)selectHealthFiles
{
    ...
    NSDateFormatter *dateFormat = [[NSDateFormatter alloc] init];
    [dateFormat setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
    NSDate *endDate = [dateFormat dateFromString:self.endTime];
