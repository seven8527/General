...
@interface TESeekDiseaseViewController ()
...
@implementation TESeekDiseaseViewController
...
- (void)tableView:(UITableView *)aTableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    TEDiseaseModel *disease = self.sortedDisease[indexPath.section][indexPath.row];

    if ([self.delegate respondsToSelector:@selector(didSelectedDiseaseId:diseaseName:)]) {
        [self.delegate didSelectedDiseaseId:disease.diseaseId diseaseName:disease.name];
    }
}
...
- (void)fetchDiseaseCategory
{
    ...
    [TEHttpTools post:URLString params:nil success:^(id responseObject) {
        ...
        TEDiseaseModel *all = [[TEDiseaseModel alloc] init];
        all.diseaseId = @"-1";
        all.name = @"全部";
        all.englishName = @"";
