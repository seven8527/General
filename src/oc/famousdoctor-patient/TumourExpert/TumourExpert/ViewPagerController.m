...
#define kDefaultTabWidth 106.50
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.selected) {
        ...
        // Draw the indicator
        [bezierPath moveToPoint:CGPointMake(25.0, rect.size.height - 1.0)];
        [bezierPath addLineToPoint:CGPointMake(rect.size.width - 25, rect.size.height - 1.0)];
...
- (void)setActiveTabIndex:(NSUInteger)activeTabIndex {
    ...
    UILabel *lable = (UILabel *) activeTabView.subviews[0];
    lable.textColor = [UIColor blackColor];
    activeTabView.selected = NO;
    
//    NSLog(@"%@",activeTabView.subviews);
    
    // Set to-be-active tab selected
    activeTabView = [self tabViewAtIndex:activeTabIndex];
    UILabel *activeLable = (UILabel *) activeTabView.subviews[0];
    activeLable.textColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"home_line_green"]];
    activeTabView.selected = YES;
    
//    //    NSLog(@"%@",tabView.subviews);
//    UILabel *lable = (UILabel *) activeTabView.subviews[0];
//    if (activeTabView.selected == YES) {
