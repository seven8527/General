...
{
    HealthInfomationDetailViewController *detail = [[HealthInfomationDetailViewController alloc]init];
    if (0 == selectIndex)
    {   // 0:咨询头条
        detail.mID= [[zxttDataArr objectAtIndex:indexPath.row] objectForKey:@"id"];
    } else {
        // 1:健康饮食
        detail.mID= [[jkysDataArr  objectAtIndex:indexPath.row] objectForKey:@"id"];
    }
