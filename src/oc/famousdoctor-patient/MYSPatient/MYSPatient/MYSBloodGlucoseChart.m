...
@interface MYSBloodGlucoseChart ()
@property (strong, nonatomic) MYSBloodGlucoseChartView * lineChart;
...
@implementation MYSBloodGlucoseChart
...
-(void)setUpChart{
    if (self.chartStyle == UUChartLineStyle) {
        if(!_lineChart){
            _lineChart = [[MYSBloodGlucoseChartView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
