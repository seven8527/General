...
#define UU_GRAY [UIColor clearColor];
...
{
    if (self.frame.size.height<216 || self.frame.size.width<kScreen_Width)
...
{
    CGFloat margin = (kScreen_Width - 315)/4;
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(point_x + margin, 99, 20, 20)];
    label.text = name;
...
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView
{
    if (self.datePickerStyle == UUDateStyle_YearMonthDayHourMinute){
        if (isIOS7) {
            ...
            [self creatValuePointXs:@[[NSString stringWithFormat:@"%f",kScreen_Width/2 - 90 - margin],[NSString stringWithFormat:@"%f",kScreen_Width/2 - 40 - margin],[NSString stringWithFormat:@"%f",kScreen_Width/2 + 20 - margin],[NSString stringWithFormat:@"%f",kScreen_Width/2 + 80 -margin]]
                          withNames:@[@"年",@"月",@"日",@"："]];
        }
        return 5;
    }
    if (self.datePickerStyle == UUDateStyle_YearMonthDay){
        if (isIOS7) {
            [self creatValuePointXs:@[[NSString stringWithFormat:@"%f",kScreen_Width/2 - 60],[NSString stringWithFormat:@"%f",(kScreen_Width/2) + 20],[NSString stringWithFormat:@"%f",kScreen_Width/2 + 50 + 50]]
                      withNames:@[@"年",@"月",@"日"]];
            }
        return 3;
    }
...
//                [pickerView reloadComponent:1];
    ...
    if ([[self returnYearColorRow:yearIndex] isEqual:[UIColor blackColor]] && [[self returnMonthColorRow:monthIndex] isEqual:[UIColor blackColor]]) {
...
//    }
        ...
        //调整为现在的时间
        for (int i=0; i<indexArray.count; i++) {
            [pickerView selectRow:[indexArray[i] integerValue] inComponent:i animated:NO];
