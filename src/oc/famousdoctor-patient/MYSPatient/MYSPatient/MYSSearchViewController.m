...
@property (nonatomic, strong)NSMutableArray *hotKey;
...
- (void)viewDidLoad
{
    ...
    _hotKey = [[NSMutableArray alloc]init];
//    [_hotKey addObject:@"患者1"];
//    [_hotKey addObject:@"患者2"];
//    [_hotKey addObject:@"患者3"];
//    [_hotKey addObject:@"患者4"];
//    [_hotKey addObject:@"患者5"];
//    [_hotKey addObject:@"患者6"];
//    [_hotKey addObject:@"患者7"];
//    [_hotKey addObject:@"患者8"];
...
-(void) getHotKey{
    ...
    [HttpTool get:[NSString stringWithFormat:@"%@hot_searchwords" ,kURL_ROOT] params:nil success:^(id responseObject) {
//        NSDictionary *dic =
        NSString * stat = [responseObject objectForKey:@"status"];
        NSLog(@"%@",responseObject);
        if ([@"1" isEqualToString:stat]) {
            
            //添加 label热词
            UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(9, 115, 80, 25)];
            label.text = @"搜索热词";
            label.textColor = [UIColor colorFromHexRGB:K00907FColor];
            label.font  = [UIFont systemFontOfSize:15.0];
            
            [self.view addSubview:label];
            
            //动态添加搜索结果
            _hotKey = [responseObject objectForKey:@"list"];
            int i ;
            for(i = 0 ; i < _hotKey.count ;i++)
            {
                int x = i/4;
                
                UIButton *button = [[UIButton alloc]initWithFrame:CGRectMake(9+(kScreen_Width-48)/4*(i%4)+10*(i%4), 145+x*38, (kScreen_Width-48)/4, 28 )];
                 button.layer.cornerRadius = 5.0;
                NSString* kStr= [[_hotKey objectAtIndex:i] objectForKey:@"name"] ;
                [button setTitle:kStr  forState:UIControlStateNormal];
                [button addTarget:self action:@selector(searchHotKey:) forControlEvents:UIControlEventTouchUpInside ];
        ...
        else if([@"-1" isEqualToString:stat])
        {
            ALERT(@"获取数据失败");

        }
        else{
          NSString *strInfo =  [NSString stringWithFormat:@"服务器返回状态异常：%@",stat];
            ALERT(strInfo);
        }
...
- (void)searchHotKey:(UIButton *) sender
{
    _searchView.text=sender.titleLabel.text;
    [self searchDoctorOrDiseaseWithSearchText:sender.titleLabel.text];
