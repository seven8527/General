...
public:    CGPoint beginPoint;
    ...
    DZBookDirection moveDirection()
    {
        ...
        if(beginPoint.y -  endPoint.y > 120)
        {
            direction = DZBookDirectionDown;
        }
        else if(endPoint.y - beginPoint.y > 120)
        {
            direction = DZBookDirectionTop;
        } else {
            direction = DZBookDirectionNone;
        }
...
////////////////
double (^kDZBottomSlideOffSet)(UIViewController*, double) = ^(UIViewController* viewController, double percent)
{
...
#define KDZBottomToggledOffSet (kScreen_Height - 88 - 64)
...
@interface DZPullDownViewController () <UIGestureRecognizerDelegate>
{
    ...
    DZTopViewControllerStatues _currentTopViewControllerStatue;
...
- (id) initWithBottom:(UIViewController *)bottomViewController top:(UIViewController *)topViewController
{
    ...
    [self setTopViewControllerStatue:DZTopViewControllerStatueBottomToggled animation:NO];
...
{
    if (statue == DZTopViewControllerStatueFullScreen | statue == DZTopViewControllerStatueBottomToggled) {
        _currentTopViewControllerStatue = statue;
    }
    if (animation) {
    ...
    else
    {
        switch (statue) {
            case DZTopViewControllerStatueFullScreen:
                _topViewController.view.frame = CGRectMake(0, 64, kScreen_Width, kScreen_Height - 64);
...
- (void) didPerformPanGestureRecoginzer:(UIGestureRecognizer*)recognizer
{
    ...
    if ([topView isKindOfClass:[UIScrollView class]]) {
        ...
        {
            scrollView.contentOffset = CGPointMake(0, -scrollView.contentInset.top - 20);
        }
    }
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan) {
        ...
        LOG(@"%f %f %f",CGRectGetMinY(self.topViewController.view.frame), kDZPullUpToggledUpOffSetTop, KDZPullDownToggledBottomOffSetTop);
        
        if (_moveData.moveDirection() != DZBookDirectionNone) {
        ...
        else
        {
            ...
            if (_currentTopViewControllerStatue == DZTopViewControllerStatueBottomToggled) {
                [self setTopViewControllerStatue:DZTopViewControllerStatueBottomToggled animation:YES];
            } else if(_currentTopViewControllerStatue == DZTopViewControllerStatueFullScreen){
                 [self setTopViewControllerStatue:DZTopViewControllerStatueFullScreen animation:YES];
            }
