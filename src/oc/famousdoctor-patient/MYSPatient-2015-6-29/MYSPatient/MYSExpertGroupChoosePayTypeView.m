//
//  MYSExpertGroupChoosePayTypeView.m
//  MYSPatient
//
...
@interface  MYSExpertGroupChoosePayTypeView ()
+ (UILabel *)buttonLabelWithText:(NSString *)text;
- (void)setActionSheetHeight:(CGFloat)height;
- (void)addButton:(UIButton *)button;
- (void)resizeButtons;
@property (nonatomic, strong) NSArray *payTypeTitleArray;
@end

@implementation MYSExpertGroupChoosePayTypeView
...
{
    MYSExpertGroupChoosePayTypeView *sheetView = [[MYSExpertGroupChoosePayTypeView alloc] initWithFrame:CGRectZero];
    if(!sheetView) return nil;
    sheetView.payTypeTitleArray = titleArray;
    ...
    for (int i = 0; i < titleArray.count; i ++)
    {
        ...
        UILabel *payLabel = [[UILabel alloc] initWithFrame:CGRectMake(kScreen_Width/2, 0, kScreen_Width/2, 44)];
    ...
    if (commitTitle)
    {
        ...
        UILabel *label = [MYSExpertGroupChoosePayTypeView buttonLabelWithText:commitTitle];
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(actionSheet:payType:)]) {
        [self.delegate actionSheet:self payType:self.payTypeTitleArray[button.tag]];
    }
