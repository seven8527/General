...
@interface MYSBloodPressureChart ()
@property (strong, nonatomic) MYSBloodPressureChartView * lineChart;
...
@implementation MYSBloodPressureChart
...
-(void)setUpChart{
    if (self.chartStyle == UUChartLineStyle) {
        if(!_lineChart){
            _lineChart = [[MYSBloodPressureChartView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
