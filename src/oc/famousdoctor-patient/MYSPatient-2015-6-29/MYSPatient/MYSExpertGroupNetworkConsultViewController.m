//
//  MYSExpertGroupNetworkConsultViewController.m
//  MYSPatient
//
...
@implementation MYSExpertGroupNetworkConsultViewController
...
- (void)viewDidLoad {
    ...
    self.patientModel = [[MYSExpertGroupPatientModel alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //    // 描述
    //    static NSString *expertGroupConsultDescription = @"expertGroupConsultDescription";
    //    MYSExpertGroupConsultDescriptionCell *expertGroupConsultDescriptionCell = [tableView dequeueReusableCellWithIdentifier:expertGroupConsultDescription];
    //    if (expertGroupConsultDescriptionCell == nil) {
    //        expertGroupConsultDescriptionCell = [[MYSExpertGroupConsultDescriptionCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:expertGroupConsultDescription];
    //    }
...
//    expertGroupConsultChooseRecordCell.detailTextLabel.textColor = [UIColor colorWithRed:196/255 green:196/255 blue:196/255 alpha:0.2];
    expertGroupConsultChooseRecordCell.detailTextLabel.backgroundColor = [UIColor clearColor];
    
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            expertGroupConsultDoctorCell.consultType = @"网络咨询";
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 1) {
        ...
        UIViewController <MYSExpertGroupDetailsDescriptionViewControllerProtocol> *viewController = [[JSObjection defaultInjector] getObject:@protocol(MYSExpertGroupDetailsDescriptionViewControllerProtocol)];
        viewController.tipText = @"本次咨询问题的详细描述及以往就医情况";
        ...
        viewController.tipText = @"希望得到医生何种帮助";
        [self.navigationController pushViewController:viewController animated:YES];
    }
...
- (void)fetchAsk
{
    ...
    NSDictionary *parameters = @{@"doctorid": self.doctorId, @"type": @"0"};
    [HttpTool post:URLString params:parameters success:^(id responseObject) {
        ...
        words = @[@{@"付款:  ": [UIFont systemFontOfSize:14]},
...
- (void)clickCommitButton
{
    ...
    if ([self validatePhone:phone patientId:self.patientModel.patientId selectHealthFiles:self.testRecordArray]) {
        ...
        viewController.consultType = @"网络咨询";
        [self.navigationController pushViewController:viewController animated:YES];
    }
}
...
// 验证电话和患者
- (BOOL)validatePhone:(NSString *)phone patientId:(NSString *)patientId selectHealthFiles:(NSArray *)selectHealthFiles
{
