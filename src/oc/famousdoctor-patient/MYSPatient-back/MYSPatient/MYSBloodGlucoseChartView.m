...
@interface MYSBloodGlucoseChartView ()
...
@implementation MYSBloodGlucoseChartView
...
-(void)setYValues:(NSArray *)yValues
{
    ...
    self.highBloodArray  = [self judgeHighWithHighBlood:yValues[0]];
    self.lowBloodArray = [self judgeLowWithHighBlood:yValues[0]];
...
- (void)addPoint:(CGPoint)point index:(NSInteger)index isShow:(BOOL)isHollow value:(CGFloat)value
{
    ...
    view.backgroundColor = [UIColor greenColor];
    if (index == 0) {
        view.backgroundColor = [UIColor colorFromHexRGB:K9D76AAColor];
...
// 血糖高数组
- (NSArray *)judgeHighWithHighBlood:(NSArray *)Blood
{
    NSMutableArray *indexArray = [NSMutableArray array];
    
    for (int i = 0; i < Blood.count; i++) {
        if ([Blood[i] integerValue] >=6.1) {
...
// 血糖低数组
- (NSArray *)judgeLowWithHighBlood:(NSArray *)Blood
{
    NSMutableArray *indexArray = [NSMutableArray array];
    
    for (int i = 0; i < Blood.count; i++) {
        if ([Blood[i] integerValue] <=3.9) {
            [indexArray addObject:[NSString stringWithFormat:@"%d",i]];
        }
    }
    return indexArray;
