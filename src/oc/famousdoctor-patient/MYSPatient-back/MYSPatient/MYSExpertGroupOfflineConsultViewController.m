//
//  MYSExpertGroupOfflineConsultViewController.m
//  MYSPatient
//
...
@implementation MYSExpertGroupOfflineConsultViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            expertGroupConsultDoctorCell.consultType = @"面对面咨询";
        ...
        if (indexPath.row == 0) {
            ...
            addNewRecordVisitingTimeCell.infoLabel.text = @"期望咨询时间";
...
- (void)featchCurrentTimeWithIndex:(NSInteger )index
{
    ...
    _consultTimeFiled.text = [NSString stringWithFormat:@"%ld-%02ld-%02ld %02ld:%02ld",(long)y,(long)m,(long)d,(long)hour,(long)min];
}
...
- (void)setupDataPicker
{
    ...
    UUDatePicker *datePicker = [[UUDatePicker alloc] initWithframe:CGRectMake(0, kScreen_Height, kScreen_Width, 200) PickerStyle:UUDateStyle_YearMonthDayHourMinute didSelected:^(NSString *year, NSString *month, NSString *day, NSString *hour, NSString *minute, NSString *weekDay) {
        _consultTimeFiled.text = [NSString stringWithFormat:@"%@-%@-%@ %@:%@",year,month,day,hour,minute];
    }];
...
- (void)clickCommitButton
{
    NSString *phone = [_phoneTextField.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
     NSString *time = [_consultTimeFiled.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
    if ([self validatePhone:phone patientId:self.patientModel.patientId selectHealthFiles:self.testRecordArray consultTime:time]) {
        ...
        viewController.consultType = @"面对面咨询";
        viewController.consultStartTime = time;
        [self.navigationController pushViewController:viewController animated:YES];
    }
}
...
- (void)fetchAsk
{
    ...
    NSDictionary *parameters = @{@"doctorid": self.doctorId, @"type": @"2"};
...
// 验证电话和患者
- (BOOL)validatePhone:(NSString *)phone patientId:(NSString *)patientId selectHealthFiles:(NSArray *)selectHealthFiles consultTime:(NSString *)consultTime
{
    ...
    if (consultTime.length == 0) {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"提示" message:@"请选择期望咨询时间" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
        [alertView show];
        return NO;
    }
