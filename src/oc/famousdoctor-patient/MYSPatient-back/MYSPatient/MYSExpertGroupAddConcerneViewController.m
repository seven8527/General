//
//  MYSExpertGroupAddConcerneViewController.m
//  MYSPatient
//
...
@interface MYSExpertGroupAddConcerneViewController () <UITableViewDataSource,UITableViewDelegate>
...
@implementation MYSExpertGroupAddConcerneViewController
...
- (void)viewDidLoad {
    ...
    // 医生列表
    UITableView *mainTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.headerView.frame), kScreen_Width, kScreen_Height - 108) style:UITableViewStylePlain];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ([tableView isEqual:self.departmentListView]) {
        ...
        MYSExpertGroupConcerneCell *expertGroupConcerneCell = [tableView dequeueReusableCellWithIdentifier:expertGroupConcerne];
        if (expertGroupConcerneCell == nil) {
            expertGroupConcerneCell = [[MYSExpertGroupConcerneCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:expertGroupConcerne];
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([tableView isEqual:self.departmentListView]) {
        ...
        if ([self.delegate respondsToSelector:@selector(expertGroupAddConcerneView:didSelectedWith:)]) {
            MYSExpertGroupDoctorModel *expertGroupDoctorModel = self.doctorArray[indexPath.row];

            [self.delegate expertGroupAddConcerneView:tableView didSelectedWith:expertGroupDoctorModel];
        }
    }
...
- (void)tapHeaderView
{
    ...
    if (self.isOpen) {
        ...
        [UIView animateWithDuration:0.1 animations:^{
            self.departmentListView.frame = CGRectMake(0, CGRectGetMaxY(self.headerView.frame), kScreen_Width, kScreen_Height - 108);
        }];
    } else {
...
- (void)findDepartment
{
    ...
    NSDictionary *parameters = @{@"attention_type": @"4",@"doctor_type": @"0"};
...
- (void)findDoctorByDepartmentId:(NSString *)departmentId scrollPositonTop:(BOOL)isScrollPositionTop
{
    ...
    NSDictionary *parameters = @{@"attention_type": @"4", @"uid": userId, @"pnid": departmentId, @"start":[NSString stringWithFormat:@"%lu",(unsigned long)self.doctorArray.count], @"end": kNumberOfPage,@"doctor_type": @"0"};
    [HttpTool get:URLString params:parameters success:^(id responseObject) {
        LOG(@"Success: %@", responseObject);
